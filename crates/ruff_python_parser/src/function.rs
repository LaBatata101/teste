...
pub(crate) fn validate_pos_params(
    _args: &(Vec<ast::ParameterWithDefault>, Vec<ast::ParameterWithDefault>),
) -> Result<(), LexicalError> {
...
type FunctionArgument = (Option<(TextSize, TextSize, Option<ast::Identifier>)>, ast::Expr);

// Parse arguments as supplied during a function/lambda *call*.
pub(crate) fn parse_arguments(_function_arguments: Vec<FunctionArgument>) -> Result<ArgumentList, LexicalError> {
    // let mut args = vec![];
    // let mut keywords = vec![];
    //
    // let mut keyword_names =
    //     FxHashSet::with_capacity_and_hasher(function_arguments.len(), BuildHasherDefault::default());
