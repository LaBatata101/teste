...
#[derive(Debug)]
pub struct Token(Tok, TextRange);
/// The result of lexing a token.
pub type LexResult = Result<Token, LexicalError>;

impl Token {
    #[inline]
    pub fn new(tok: Tok, range: TextRange) -> Token {
        Self(tok, range)
    }

    #[inline]
    pub fn kind(&self) -> &Tok {
        &self.0
    }
    ...
    I: Iterator<Item = LexResult>,
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        let result = match self.inner.next()? {
            Ok(Token(tok, range)) => Ok(Token(tok, range + self.start_offset)),
...
impl<I> ExactSizeIterator for LexStartsAtIterator<I> where I: Iterator<Item = LexResult> + ExactSizeIterator {}
...
impl<'source> Lexer<'source> {
    ...
    fn lex_identifier(&mut self, first: char) -> Result<Tok, LexicalError> {
        ...
        let keyword = match text {
            ...
            "with" => Tok::With,
            "yield" => Tok::Yield,
            _ => return Ok(Tok::Name { name: text.to_string() }),
        };
    ...
    fn lex_decimal_number(&mut self, first_digit_or_dot: char) -> Result<Tok, LexicalError> {
        ...
        if is_float {
            ...
            if self.cursor.eat_if(|c| matches!(c, 'j' | 'J')).is_some() {
                Ok(Tok::Complex { real: 0.0, imag: value })
            } else {
                Ok(Tok::Float { value })
            }
        } else {
                    ...
                    if let Some(fstring) = self.fstrings.current() {
                        ...
                        if fstring.quote_char() == quote && !fstring.is_triple_quoted() {
                            return Err(LexicalError {
                                ...
                                // TODO: check if it's correct
                                location: TextRange::new(self.token_start(), self.offset() - fstring.quote_size()),
                            });
                        }
                    }
                    return Err(LexicalError {
                        error: LexicalErrorType::OtherError("EOL while scanning string literal".to_owned()),
                        // TODO: check if it's correct
                        location: TextRange::new(self.token_start(), self.offset() - TextSize::new(1)),
                    });
                }
                Some(c) if c == quote => {
                    if triple_quoted {
            ...
            if !fstring.is_in_expression(self.nesting) {
                match self.lex_fstring_middle_or_end() {
                    Ok(Some(tok)) => {
                        ...
                        return Ok(Token(tok, self.token_range()));
                    }
                    Err(e) => {
            ...
            match self.indentations.current().try_compare(indentation) {
                Ok(Ordering::Greater) => {
                    ...
                    self.indentations
                        ...
                        // TODO: check if range it's correct
                        .map_err(|_| LexicalError::new(LexicalErrorType::IndentationError, self.token_range()))?;
                    return Ok(Token(Tok::Dedent, TextRange::empty(offset)));
                }
                Ok(_) => {}
                Err(_) => {
                    // TODO: check if range it's correct
    ...
    fn handle_indentation(&mut self, indentation: Indentation) -> Result<Option<Token>, LexicalError> {
        let token = match self.indentations.current().try_compare(indentation) {
            ...
            Ok(Ordering::Greater) => {
                ...
                Some(Token(Tok::Dedent, TextRange::empty(self.offset())))
            }
            ...
            Ok(Ordering::Less) => {
                ...
                Some(Token(Tok::Indent, self.token_range()))
            }
            Err(_) => {
                return Err(LexicalError {
    ...
    fn consume_end(&mut self) -> Result<Token, LexicalError> {
        ...
        if !self.state.is_new_logical_line() {
            ...
            Ok(Token(Tok::Newline, TextRange::empty(self.offset())))
        }
        // Next, flush the indentation stack to zero.
        else if self.indentations.dedent().is_some() {
            Ok(Token(Tok::Dedent, TextRange::empty(self.offset())))
        } else {
            Ok(Token(Tok::EndOfFile, TextRange::empty(self.offset())))
        }
    }
    ...
    fn consume_ascii_character(&mut self, c: char) -> Result<Token, LexicalError> {
        let token = match c {
            ...
            '#' => return Ok(Token(self.lex_comment(), self.token_range())),
            ...
            '=' => {
                if self.cursor.eat_char('=') {
                    ...
                    return Ok(Token(Tok::Equal, self.token_range()));
                }
            }
            '+' => {
                if self.cursor.eat_char('=') {
            ...
            c @ ('%' | '!') if self.mode == Mode::Ipython && self.state.is_after_equal() && self.nesting == 0 => {
...
mod tests {
    ...
    fn test_ipython_escape_command_line_continuation_with_unix_eol_and_eof() {
        assert_debug_snapshot!(ipython_escape_command_line_continuation_with_eol_and_eof(UNIX_EOL));
    }

    #[test]
    fn test_ipython_escape_command_line_continuation_with_mac_eol_and_eof() {
        assert_debug_snapshot!(ipython_escape_command_line_continuation_with_eol_and_eof(MAC_EOL));
    }

    #[test]
    fn test_ipython_escape_command_line_continuation_with_windows_eol_and_eof() {
        assert_debug_snapshot!(ipython_escape_command_line_continuation_with_eol_and_eof(WINDOWS_EOL));
    }
...
def f(arg=%timeit a = b):
    ...
    #[test]
    fn test_numbers() {
        let source = "0x2f 0o12 0b1101 0 123 123_45_67_890 0.2 1e+2 2.1e3 2j 2.2j 000 0x995DC9BBDF1939FA";
    ...
    fn test_invalid_leading_zero_big() {
        let source = "0252222222222222522222222222225222222222222252222222222222522222222222225222222222222";
...
"
    ...
    fn test_emoji_identifier() {
        ...
        match lexed.as_slice() {
            [Err(error)] => {
                assert_eq!(error.error, LexicalErrorType::UnrecognizedToken { tok: '🐦' });
            }
