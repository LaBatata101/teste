...
pub use parser::{
    parse, parse_expression, parse_expression_starts_at, parse_ok_tokens, parse_program, parse_starts_at, parse_suite,
...
pub fn locate_cmp_ops(expr: &Expr, source: &str) -> Vec<LocatedCmpOp> {
    ...
    let mut tok_iter = lexer::lex(&parenthesized_contents, Mode::Expression)
        .flatten()
        .skip(1)
        .map(|token| (token.kind().clone(), token.range() - TextSize::from(1)))
        ...
        match tok {
            ...
            Tok::Is => {
                let op = if let Some((_, next_range)) = tok_iter.next_if(|(tok, _)| tok.is_not()) {
                    LocatedCmpOp::new(TextRange::new(range.start(), next_range.end()), CmpOp::IsNot)
                } else {
...
mod tests {
    ...
    fn extract_cmp_op_location() -> Result<()> {
        ...
        assert_eq!(
            ...
            vec![LocatedCmpOp::new(TextSize::from(2)..TextSize::from(4), CmpOp::Eq)]
        );
        ...
        assert_eq!(
            locate_cmp_ops(&expr, contents),
            vec![LocatedCmpOp::new(TextSize::from(2)..TextSize::from(4), CmpOp::NotEq)]
        );
        ...
        assert_eq!(
            locate_cmp_ops(&expr, contents),
            vec![LocatedCmpOp::new(TextSize::from(2)..TextSize::from(4), CmpOp::Is)]
        );
        ...
        assert_eq!(
            locate_cmp_ops(&expr, contents),
            vec![LocatedCmpOp::new(TextSize::from(2)..TextSize::from(8), CmpOp::IsNot)]
        );
        ...
        assert_eq!(
            locate_cmp_ops(&expr, contents),
            vec![LocatedCmpOp::new(TextSize::from(2)..TextSize::from(4), CmpOp::In)]
        );
        ...
        assert_eq!(
            locate_cmp_ops(&expr, contents),
            vec![LocatedCmpOp::new(TextSize::from(2)..TextSize::from(8), CmpOp::NotIn)]
        );
