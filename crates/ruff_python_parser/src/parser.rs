...
use crate::lexer::{lex, lex_starts_at, Token};
use crate::{
...
/// assert!(expr.is_ok());
/// ```
pub fn parse_expression_starts_at(source: &str, source_path: &str, offset: TextSize) -> Result<ast::Expr, ParseError> {
...
/// assert!(program.is_ok());
/// ```
pub fn parse_starts_at(source: &str, mode: Mode, source_path: &str, offset: TextSize) -> Result<Mod, ParseError> {
...
pub fn parse_tokens(
    ...
    parse_filtered_tokens(
        lxr.filter_ok(|token| !matches!(token.kind(), Tok::Comment { .. } | Tok::NonLogicalNewline)),
        source,
        mode,
        source_path,
    )
...
pub fn parse_ok_tokens(
    lxr: impl IntoIterator<Item = Token>,
    source: &str,
    mode: Mode,
    source_path: &str,
) -> Result<Mod, ParseError> {
    let lxr = lxr
        .into_iter()
        .filter(|token| !matches!(token.kind(), Tok::Comment { .. } | Tok::NonLogicalNewline));
    let marker_token = Token::new(Tok::start_marker(mode), TextRange::default());
    let lexer = iter::once(marker_token)
        .chain(lxr)
        .map(|token| (token.range().start(), token.kind().clone(), token.range().end())); // TODO: remove clone
...
fn parse_filtered_tokens(
    ...
    python::TopParser::new()
        .parse(
            ...
            mode,
            // TODO: remove clone
            lexer.map_ok(|token| (token.range().start(), token.kind().clone(), token.range().end())),
        )
        .map_err(|e| parse_error_from_lalrpop(e, source_path))
}
...
impl fmt::Display for ParseError {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        write!(f, "{} at byte offset {}", &self.error, u32::from(self.offset.start()))
    }
}
...
fn parse_error_from_lalrpop(_err: LalrpopError<TextSize, Tok, LexicalError>, _source_path: &str) -> ParseError {
    todo!()
...
%timeit a == 1
    "#
        ...
        assert_eq!(
            parse_err.to_string(),
            "IPython escape commands are only allowed in `Mode::Ipython` at byte offset 6".to_string()
        );
    }
