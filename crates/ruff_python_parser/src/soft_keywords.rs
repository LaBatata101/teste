    ...
    I: Iterator<Item = LexResult>,
{
    ...
    fn next(&mut self) -> Option<LexResult> {
        ...
        if let Some(Ok(token)) = next.as_ref() {
            ...
            match token.kind() {
                ...
                Tok::Match | Tok::Case => {
                    if self.start_of_line {
                        ...
                        while let Some(Ok(token)) = self.underlying.peek() {
                            match token.kind() {
                                Tok::Newline => break,
                        ...
                        if !seen_colon {
                            next = Some(Ok(Token::new(soft_to_name(token.kind()), token.range())));
                        }
                    } else {
                        next = Some(Ok(Token::new(soft_to_name(token.kind()), token.range())));
                    }
                ...
                Tok::Type => {
                    if self.start_of_line {
                        ...
                        if let Some(Ok(token)) = self.underlying.peek() {
                            if matches!(
                                token.kind(),
                                Tok::Name { .. } |
        ...
        self.start_of_line = next.as_ref().is_some_and(|lex_result| {
            lex_result.as_ref().is_ok_and(|token| {
                if matches!(token.kind(), Tok::NonLogicalNewline | Tok::Comment { .. }) {
