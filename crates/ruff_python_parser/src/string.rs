...
impl<'a> StringParser<'a> {
    fn new(source: &'a str, kind: StringKind, start: TextRange) -> Self {
        Self {
            rest: source,
            kind,
            location: start,
    ...
    #[inline]
    fn get_pos(&self) -> TextRange {
        self.location
    }

    #[inline]
    fn range(&self, start_location: TextSize) -> TextRange {
        TextRange::new(start_location, self.location.end())
    }
    ...
    fn parse_unicode_name(&mut self) -> Result<char, LexicalError> {
        ...
        let Some(close_idx) = self.rest.find('}') else {
            return Err(LexicalError::new(LexicalErrorType::StringError, self.get_pos()));
        };

        let name_and_ending = self.skip_bytes(close_idx + 1);
        let name = &name_and_ending[..name_and_ending.len() - 1];

        unicode_names2::character(name).ok_or_else(|| LexicalError::new(LexicalErrorType::UnicodeError, start_pos))
    }
    ...
    fn parse_bytes(&mut self) -> Result<StringType, LexicalError> {
        ...
        //                     LexicalErrorType::OtherError("bytes can only contain ASCII literal characters".to_string()),
        //                     self.get_pos(),
        //                 ));
        //             }
        //             content.push(ch);
...
pub(crate) fn parse_fstring_middle(_source: &str, _is_raw: bool, _start_location: TextSize) -> Result<Expr, LexicalError> {
    todo!()
    // let kind = if is_raw {
...
pub(crate) fn concatenate_strings(_strings: Vec<StringType>, _range: TextRange) -> Result<Expr, LexicalError> {
    ...
    //     return Err(LexicalError {
    //         error: LexicalErrorType::OtherError("cannot mix bytes and nonbytes literals".to_owned()),
    ...
    //                     Expr::FormattedValue { .. } => {
    //                         if !current.is_empty() {
    //                             deduped.push(take_current(&mut current, current_start, current_end, is_unicode));
    //                         }
    //                         deduped.push(value);
    //                         is_unicode = false;
    //                     }
...
impl std::fmt::Display for FStringErrorType {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        use FStringErrorType::{
            InvalidConversionFlag, LambdaWithoutParentheses, SingleRbrace, UnclosedLbrace, UnterminatedString,
            UnterminatedTripleQuotedString,
        };
        match self {
